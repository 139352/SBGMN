...
parser.add_argument('--a_emb_size', type=int, default=48, help='Embeding A Size')
parser.add_argument('--b_emb_size', type=int, default=48, help='Embeding B Size')
...
parser.add_argument('--dropout', type=float, default=0.6, help='Dropout')#0.5
...
#             Wh = torch.mm(X, self.W[i])  # X * W[i]
#             attn_scores = torch.mm(Wh, X.t())  # 注意力分数矩阵
#             attn_scores = attn_scores + torch.log(A.unsqueeze(0) + 1e-10)  # 加入注意力权重
#             attn_scores = F.leaky_relu(attn_scores, self.alpha)  # LeakyReLU 激活
#
#             # 转化为注意力权重
...
#
#             # 注意力加权聚合节点特征
#             h = torch.mm(attn_weights, X)
#             heads.append(h)
#
#         # 合并多头输出并添加残差连接
#         output = torch.cat(heads, dim=1)
#         output = F.elu(output)  # 使用激活函数（例如，ELU）
#         output += X  # 残差连接
#
#         return output
...
#         # GAT 层 1
#         self.gat1 = GATConvLayer(in_features, hidden_features, num_heads)
#         # GAT 层 2
#         self.gat2 = GATConvLayer(hidden_features * num_heads, out_features, 1)  # 最后一层只有一个注意力头
#
#     def forward(self, X, A):
#         # X: 节点特征矩阵 (N, in_features)
#         # A: 标准化邻接矩阵 (N, N)
...
class SBGNNLayer2(nn.Module):
    def __init__(self, edgelist_a_b_pos, edgelist_a_b_neg, edgelist_b_a_pos, edgelist_b_a_neg, dataset_name=args.dataset_name,
                 emb_size_a=48, emb_size_b=48, aggregator1=MeanAggregator):
    ...
    def forward(self, feature_a, feature_b):
        ...
        tensor_a_b_pos = torch.tensor([float(m_a_b_pos[key]) for key in sorted_keys_a_b_pos], dtype=torch.float).view(-1, 1)
        ...
        tensor_a_b_neg = torch.tensor([float(m_a_b_neg[key]) for key in sorted_keys_a_b_neg], dtype=torch.float).view(-1, 1)
        ...
        tensor_b_a_pos = torch.tensor([float(m_b_a_pos[key]) for key in sorted_keys_b_a_pos], dtype=torch.float).view(-1, 1)
        tensor_b_a_pos = tensor_b_a_pos[node_num_a:,:].to(args.device)
        ...
        tensor_b_a_neg = torch.tensor([float(m_b_a_neg[key]) for key in sorted_keys_b_a_neg], dtype=torch.float).view(-1, 1)
...
class SBGNNLayer(nn.Module):
    def __init__(self, edgelist_a_b_pos, edgelist_a_b_neg, edgelist_b_a_pos, edgelist_b_a_neg,
                 dataset_name=args.dataset_name, emb_size_a=48, emb_size_b=48, aggregator=AttentionAggregator):
...
class SBGNN(nn.Module):
    def __init__(self, edgelists,
                 dataset_name=args.dataset_name, layer_num=1, emb_size_a=48, emb_size_b=48):
        ...
        self.multi_view_model = GraphSAGE_NET(input_dim=50, hidden=32, classes=50)

        self.layers1 = nn.ModuleList(
            [SBGNNLayer(edgelist_a_b_pos, edgelist_a_b_neg, edgelist_b_a_pos, edgelist_b_a_neg,
                        dataset_name=dataset_name, emb_size_a=50, emb_size_b=50, aggregator=AttentionAggregator) for _ in
             range(layer_num)]
        )

        self.layers2 = nn.ModuleList(
            [SBGNNLayer2(edgelist_a_b_pos, edgelist_a_b_neg, edgelist_b_a_pos, edgelist_b_a_neg,
                        dataset_name=dataset_name, emb_size_a=50, emb_size_b=50, aggregator1=MeanAggregator) for _ in
             range(layer_num)]
        )
    ...
    def multi_view(self, emb1):
        if emb1.device != 'cpu':
            emb_view = lil_matrix(emb1.cpu().detach().numpy())
        else:
    ...
    def forward(self, edge_lists):
        ...
        emb_view = self.multi_view(emb1)    #同类型节点特征构造------multi-view
        ...
        return torch.sigmoid(y), emb_view_a, emb_view_b, embedding_a1, embedding_b1, embedding_a2, embedding_b2
...
def run():
    ...
    for epoch in tqdm(range(1, args.epoch + 2)):
        ...
        model.train()
        optimizer.zero_grad()
        pred_y, emb_view_a, emb_view_b, embedding_a, embedding_b, embedding_a2, embedding_b2 = model(train_edgelist)
        # print('debug', pred_y)
        ...
        # loss = 1.3 * loss1 + 0.35 * loss2 + 0.35 * loss3
        loss.backward()
        optimizer.step()
        ...
        if True:
            ...
            res_cur.update(res)
            pred_val_y, emb_view_a, emb_view_b, embedding_a, embedding_b, embedding_a2, embedding_b2 = model(val_edgelist)
            res = test_and_val(pred_val_y, val_y, mode='val', epoch=epoch)
            res_cur.update(res)
            pred_test_y, emb_view_a, emb_view_b, embedding_a, embedding_b, embedding_a2, embedding_b2  = model(test_edgelist)
