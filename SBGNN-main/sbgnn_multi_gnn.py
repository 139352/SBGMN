...
class SBGNNLayer(nn.Module):
    ...
    def forward(self, feature_all):
        ...
        tensor_pos = F.normalize(tensor_pos, p=2, dim=0)
        tensor_pos = tensor_pos.to(args.device)
        ...
        tensor_neg = F.normalize(tensor_neg, p=2, dim=0)
        tensor_neg = tensor_neg.to(args.device)
        ...
        new_feature = torch.cat([feature_all, tensor_pos, tensor_neg], dim=1)
...
class SBGNN(nn.Module):
    def __init__(self, edgelists,
        ...
        self.layers1 = nn.ModuleList(
            [SBGNNLayer(edgelist_pos, edgelist_neg,
                    dataset_name=dataset_name, emb_size_a=32, emb_size_b=32, aggregator=aggregator1) for _ in range(1)]
        )
    ...
    def get_embeddings(self):
        ...
        # emb_a = self.out_mlp_layer(emb_a)
        emb_b = self.features_b(torch.arange(self.set_b_num).to(args.device))
        # emb_b = self.out_mlp_layer(emb_b)
        all_emb = torch.cat([emb_a, emb_b], dim=0)
        for m in self.layers1:
            all_emb = m(all_emb) # emb = [emb1, emb2]
        return all_emb     #这一步就是在生成最终的节点特征

    def multi_gnn(self, emb, edgelist):
        # edgelist[:, 1] = edgelist[:,1]+515
        edge_index = np.vstack([edgelist[:, 0], edgelist[:, 1]])
        edge_index = torch.tensor(edge_index).to(args.device)
        ...
        emb1 = self.multi_gnn_model1(emb, edge_index).to(args.device)
    ...
    def forward(self, edgelist):
        emb_all = self.get_embeddings()
        x1, x2, adjacency_matrix1 = self.multi_gnn(emb_all, edgelist)
        # emb_all = self.gnn(emb_all)
        # emb_a = emb_all[:self.set_a_num]
        # emb_b = emb_all[self.set_a_num:]

        # x1.requires_grad_(True)
        # concatenated_features = x1.cpu()
        concatenated_features = x1
...
def run():
    ...
    for epoch in tqdm(range(1, args.epoch + 2)):   #这行代码使用了 tqdm 函数创建了一个进度条，用于迭代训练过程的每个 epoch.tqdm 是一个 Python 库，用于在命令行中创建美观的进度条
        ...
        loss_all = model.loss_all(pred_y, train_y)
        loss_multi_gnn = model.loss_multi_gnn_loss(emb1, emb2, adjacency_matrix1)
        loss = 0.7 * loss_all + 0.3 * loss_multi_gnn
        ...
        # val/test
            ...
            count_zeros = torch.sum(pred_y < 0.5).item()
            count_zero = torch.sum(train_y < 0.5).item()
            count_ones = torch.sum(pred_y >= 0.5).item()
            ...
            count_zeros_val = torch.sum(pred_val_y < 0.5).item()
            count_zero_val = torch.sum(val_y < 0.5).item()
            count_ones_val = torch.sum(pred_val_y >= 0.5).item()
            ...
            count_zeros_test = torch.sum(pred_test_y < 0.5).item()
            count_zero_test = torch.sum(test_y < 0.5).item()
            count_ones_test = torch.sum(pred_test_y >= 0.5).item()
