...
class SBGNN(nn.Module):
    def __init__(self, edgelists,
        ...
        self.update_func = nn.Sequential(  # self.update_func 将会是一个由以下几个层组成的神经网络模型。这个模型可能被用于在图神经网络中进行节点特征的更新或聚合操作
            ...
            nn.PReLU(),
            nn.Linear(emb_size_a+2, emb_size_b+2)
        ...
        self.classifier = SimpleClassifier(input_size=34, output_size=1)
        self.classifier_a = SimpleClassifier(input_size=34, output_size=1)
        self.classifier_b = SimpleClassifier(input_size=34, output_size=1)
    ...
    def multi_view(self, emb1):
        ...
        emb_view = lil_matrix(emb1.detach().numpy())
        delta = 0.375
        ...
        emb_view = self.multi_view_model(emb1, edge_index)
    ...
    def multi_gnn(self, emb, edgelist):
        ...
        # np.fill_diagonal(adjacency_matrix1, 0)

        # max_column_indices = torch.argmax(adjacency_matrix1, dim=1)
        # for i in range(adjacency_matrix1.shape[0]):
        #     loss_1 = loss_1 + kl(F.log_softmax(emb1[i], dim=0), F.softmax(emb1[max_column_indices[i]], dim=0)).sum(
        #                                  dim=0) / adjacency_matrix1.shape[0]

        # # scores = torch.mm(emb1.t(), torch.mm(adjacency_matrix1, emb1))
        # # _, indices = scores.flatten().topk(60)  # 获取最大权重的节点对的索引
        #
        # # criterion1 = nn.MSELoss()
        #
        # # 计算目标函数（无监督学习的目标是使得正样本嵌入靠近）
        ...
        return emb1, emb2, adjacency_matrix1


    def forward(self, edgelist):
        emb1 = self.get_embeddings()
        emb_view = self.multi_view(emb1)

        emb_view = self.update_func(emb_view)
        ...
        x2, emb2, adjacency_matrix1 = self.multi_gnn(emb1, edgelist)
        # print("线性层之前emb：", emb)

        x2 = self.update_func(x2.cpu())
        emb2 = self.update_func(emb2.cpu())

        # print("线性层之后emb：", emb)
        ## embedding1.requires_grad_(True)
        x2.requires_grad_(True)
        emb_view.requires_grad_(True)
        ...
        concatenated_features_view = emb_view.cpu()
        ...
        logits_a = self.classifier_a(concatenated_features_view[:515, :])
        logits_b = self.classifier_b(concatenated_features_view[515:, :])
        ### logits_view = self.classifier(concatenated_features_view)
        # probs = logits.squeeze(dim=1)
        ### probs_view = logits_view.squeeze(dim=1)
        # probs = torch.sigmoid(logits)
        y = torch.einsum("ij, ij->i", [logits[edgelist[:, 0]], logits[edgelist[:, 1]]])
        ...
        return y, x2, emb2, adjacency_matrix1, logits_a, logits_b


    def loss_multi_view_loss(self, logits_a, logits_b):
        label_a = torch.zeros_like(logits_a)
        label_b = torch.ones_like(logits_b)
        loss_a = F.cross_entropy(logits_a, label_a)
        loss = loss_a + F.cross_entropy(logits_b, label_b)
        return loss

    def loss_multi_gnn_loss(self, emb1, emb2, adjacency_matrix1):
        kl = nn.KLDivLoss(reduction='none').to(device)
        loss_1 = kl(F.log_softmax(emb1, dim=1), F.softmax(emb2.detach(), dim=1)).sum(dim=1) / emb1.shape[0]
        ...
        max_column_indices = torch.argmax(adjacency_matrix1, dim=1)
        for i in range(adjacency_matrix1.shape[0]):
            weight = adjacency_matrix1[i][max_column_indices[i]]/100
            total_loss += weight * similarity_loss(emb1[i], emb1[max_column_indices[i]])
        total_loss = total_loss / adjacency_matrix1.shape[0]
        loss_1 = loss_1.to(device) + total_loss
        return loss_1
...
def run():
    ...
    model = SBGNN(edgelists, dataset_name=args.dataset_name, layer_num=args.gnn_layer_num, aggregator1=MeanAggregator)
    ...
    for epoch in tqdm(range(1, args.epoch + 2)):   #这行代码使用了 tqdm 函数创建了一个进度条，用于迭代训练过程的每个 epoch.tqdm 是一个 Python 库，用于在命令行中创建美观的进度条
        ...
        pred_y, emb1, emb2, adjacency_matrix1, logits_a, logits_b = model(train_edgelist)
        loss = 0.5 * model.loss_all(pred_y, train_y) + \
               0.2 * model.loss_multi_gnn_loss(emb1, emb2, adjacency_matrix1) + \
               0.3 * model.loss_multi_view_loss(logits_a, logits_b)
        ...
        # val/test
            ...
            count_zeros = torch.sum(pred_y == 0).item()
            count_zero = torch.sum(train_y == 0).item()
            count_ones = torch.sum(pred_y == 1).item()
            print("Number of elements equal to 0:", count_zeros)
            print("Number of elements equal to 1:", count_ones)
            print("train_y------Number of elements equal to 0:", count_zero)

            res_cur.update(res)   #res_cur 是一个字典，它可能包含了一些先前的评估结果。res 是另一个字典，其中包含了新的评估结果，例如在验证集或测试集上计算得到的AUC等指标。
            pred_val_y, emb1, emb2, adjacency_matrix1, logits_a, logits_b = model(val_edgelist)  #   update 方法用于将字典 res 中的键值对添加到字典 res_cur 中。如果 res 中的键在 res_cur 中不存在，则将该键值对添加到 res_cur 中；如果 res_cur 中已经有相同的键，则用 res 中的值更新 res_cur 中的值。
            res = test_and_val(pred_val_y, val_y, mode='val', epoch=epoch)

            count_zeros_val = torch.sum(pred_val_y == 0).item()
            count_zero_val = torch.sum(val_y == 0).item()
            count_ones_val = torch.sum(pred_val_y == 1).item()
            print("Number of elements equal to 0  val:", count_zeros_val)
            print("Number of elements equal to 1  val:", count_ones_val)
            print("val_y------Number of elements equal to 0:", count_zero_val)

            res_cur.update(res)
            pred_test_y, emb1, emb2, adjacency_matrix1, logits_a, logits_b = model(test_edgelist)
            res = test_and_val(pred_test_y, test_y, mode='test', epoch=epoch)

            count_zeros_test = torch.sum(pred_test_y == 0).item()
            count_zero_test = torch.sum(test_y == 0).item()
            count_ones_test = torch.sum(pred_test_y == 1).item()
            print("Number of elements equal to 0  test:", count_zeros_test)
            print("Number of elements equal to 1  test:", count_ones_test)
            print("val_y------Number of elements equal to 0:", count_zero_test)
