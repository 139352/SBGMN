...
class SBGNN(nn.Module):
    def __init__(self, edgelists,
        ...
        self.multi_view_model = GraphSAGE_NET(input_dim=35, hidden=128, output_dim=35)
    ...
    def multi_gnn_loss(self, embedding_a, embedding_b, embedding_a2, embedding_b2):
        ...
        # top_indices = torch.topk(adjacency_matrix1, k=10, dim=1)
        ...
        top_values, top_indices = torch.topk(adjacency_matrix1, k=10, dim=0)

        # 计算相似度损失
        adjacency_loss = 0.0
        for i in range(top_values.shape[0]):  # 遍历每一列
            for j in range(top_values.shape[1]):  # 遍历前十个最大值的索引
                # weight = top_values[i, j] / 100
                weight = top_values[i, j]
                adjacency_loss += weight * similarity_loss(emb1[i], emb1[top_indices[i, j]])

        # total_loss = total_loss/emb1.shape[0]
        # total_loss = total_loss/10
        adjacency_loss = adjacency_loss / (top_values.shape[0] * top_values.shape[1]) # to mean

        # loss = loss_1.to(args.device) + adjacency_loss
        loss = adjacency_loss
...
def run():
    ...
    # lambda_f = lambda cur_iter: \
    #         cur_iter / warm_up_iter if cur_iter < warm_up_iter \
    #             else (lr_min + 0.5 * (lr_max - lr_min) *
    #                   (1.0 + math.cos((cur_iter - warm_up_iter) /
    #                   (T_max - warm_up_iter) * math.pi))) / 0.1
    # scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_f)
