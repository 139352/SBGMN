...
from common import weisfeiler_lehman_labels_from_edge_list, weisfeiler_lehman_labels_from_edge_list_gpu, generate_new_adj
import torch
...
class MeanAggregator(nn.Module):
    ...
    def forward(self, edge_dic_list: dict, node_num):
        ...
        # edges = []
        # for node in edge_dic_list:
        #     neighs = torch.tensor(edge_dic_list[node], dtype=torch.long, device=self.out_mlp_layer[0].weight.device)
        #     a = torch.tensor([node] * len(neighs), dtype=torch.long, device=self.out_mlp_layer[0].weight.device).view(
        #         -1, 1)
        #     edges.append(torch.cat([a, neighs.view(-1, 1)], dim=1))
        #
        # max_iterations = 3
        # output_extra_features = weisfeiler_lehman_labels_from_edge_list_gpu(edges, max_iterations, node_num)
...
class SBGNNLayer2(nn.Module):
    def __init__(self, edgelist_a_b_pos, edgelist_a_b_neg, edgelist_b_a_pos, edgelist_b_a_neg, dataset_name, dropout=0.5,
...
class SBGNNLayer(nn.Module):
    def __init__(self, edgelist_a_b_pos, edgelist_a_b_neg, edgelist_b_a_pos, edgelist_b_a_neg,
                 dataset_name, dropout=0.5, emb_size_a=35, emb_size_b=35, aggregator=AttentionAggregator):
...
class SBGNN(nn.Module):
    def __init__(self, edgelists, dataset_name, layer_num, view_hidden=64, dropout=0.5,
                 view_relate_rate = 0.05, emb_size_a=32, emb_size_b=32, device='cuda'):
        ...
        self.multi_left_view_model = GraphSAGE_NET(input_dim=35, hidden=view_hidden, output_dim=35, dropout=dropout)  # 1316
        self.multi_right_view_model = GraphSAGE_NET(input_dim=35, hidden=view_hidden, output_dim=35, dropout=dropout)  # 1316

        self.layers_gnn1 = nn.ModuleList(
            [SBGNNLayer(edgelist_a_b_pos, edgelist_a_b_neg, edgelist_b_a_pos, edgelist_b_a_neg,
                dataset_name=dataset_name, dropout=dropout, emb_size_a=35, emb_size_b=35, aggregator=AttentionAggregator) for _ in
             range(layer_num)]
        )
        ...
        self.layers2 = nn.ModuleList(
            [SBGNNLayer2(edgelist_a_b_pos, edgelist_a_b_neg, edgelist_b_a_pos, edgelist_b_a_neg,
                dataset_name=dataset_name, dropout=dropout, emb_size_a=33, emb_size_b=33, aggregator1=MeanAggregator) for _ in
             range(1)]
        )
    ...
    def multi_view(self, emb_all):
        ...
        # delta = 0.375 * 2
        # matrix = generate_new_adj(emb_view.todense(), delta).astype(np.int16)  # todo
        # tensor = torch.tensor(matrix.todense(), dtype=torch.int32)
        ...
        # left_edge_index = torch.nonzero(tensor[:self.set_a_num, :self.set_a_num],
        #                            as_tuple=False).t().to(args.device)
        left_emb_view = self.multi_left_view_model(emb_all[:self.set_a_num], left_edge_index)

        # right_edge_index = torch.nonzero(tensor[self.set_a_num:, self.set_a_num:],
        #                                 as_tuple=False).t().to(args.device)
        right_emb_view = self.multi_left_view_model(emb_all[self.set_a_num:], right_edge_index)
        # print(left_edge_index.shape[-1], right_edge_index.shape[-1])
        # print(len(top_k_row_indices_l), len(top_k_row_indices_r))
...
class SBGNN_v1(nn.Module):
    def __init__(self, edgelists, dataset_name, layer_num, view_hidden=64, dropout=0.5,
                 view_relate_rate = 0.05, emb_size_a=32, emb_size_b=32, device='cuda'):
        super(SBGNN_v1, self).__init__()
...
class SBGNNOnlyView(nn.Module):
    def __init__(self, edgelists, dataset_name, layer_num, view_hidden=64, dropout=0.5,
                 view_relate_rate = 0.05, emb_size_a=32, emb_size_b=32, device='cuda'):
        super(SBGNNOnlyView, self).__init__()
    ...
    def forward(self, edge_lists):
        ...
        embedding_a = emb_view_a
        embedding_b = emb_view_b
...
class SBGNNOnlyGNN(nn.Module):
    def __init__(self, edgelists, dataset_name, layer_num, view_hidden=64, dropout=0.5,
                 view_relate_rate = 0.05, emb_size_a=32, emb_size_b=32, device='cuda'):
        super(SBGNNOnlyGNN, self).__init__()
